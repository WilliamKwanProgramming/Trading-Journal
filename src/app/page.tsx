...
import { MetricsToggle } from "@/components/metrics-toggle";
import { StatisticalEdge } from "@/components/statistical-edge";
...
function wholeMoney(value: number, currency = "USD") {
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <div className="layout-grid">
        ...
        <div>
          <section className="stats"><Stat label="Realized P&L · CAD" value={wholeMoney(data.metrics.realizedPnlCad, "CAD")} detail="Closed and booked P&L" /><Stat label="Open marked P&L · CAD" value={wholeMoney(data.metrics.openMarkedPnlCad, "CAD")} detail="Unrealized at latest mark" /><Stat label="Trades" value={String(data.metrics.totalTrades)} detail={`${data.metrics.closedTrades} closed · ${data.metrics.totalExecutions} executions`} /><Stat label="Win rate" value={pct(data.metrics.winRate)} detail={`${data.metrics.wins} wins · ${data.metrics.losses} losses`} /><Stat label="Max drawdown · CAD" value={wholeMoney(-data.metrics.maxDrawdownCad, "CAD")} detail="Largest peak-to-trough P&L" /></section>
          ...
          <MetricsToggle><StatisticalEdge data={data.statisticalEdge} /></MetricsToggle>
